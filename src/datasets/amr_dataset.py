...
class AMRDataset(DatasetBase):
    ...
    def __init__(
        ...
        self.seed = seed
        self.pos_scale_factor = pos_scale
        self.pos_scale = None  # will be populated after stats load
        ...
        data_source = os.path.join(global_root.as_posix(), split)
        
        self.data_source = data_source
        self.logger.info(f"data_source (global): '{data_source}'")

        self._stats_id = self._build_stats_id(global_root, self.name)
        self._stats_path = STATS_DIR / f"{self._stats_id}_train.pt"
        ...
        # Compute normalization statistics (always derived from training data)
        if self.split == "train":
            if is_data_rank0() and not self._stats_path.exists():
                stats = self._compute_normalization_stats()
                torch.save(stats, self._stats_path)
            barrier()
            ...
            if not self._stats_path.exists():
                raise FileNotFoundError(
                    f"Expected normalization stats at {self._stats_path}. Instantiate the training split first."
                )
            stats = torch.load(self._stats_path)
    ...
    def _compute_normalization_stats(self):
        """Compute normalization statistics across all training simulations."""
        self.logger.info("Computing AMR normalization statistics from training data...")

        pos_min = np.full(2, np.inf, dtype=np.float64)
        pos_max = np.full(2, -np.inf, dtype=np.float64)

        log_mean = np.zeros(3, dtype=np.float64)
        log_M2 = np.zeros(3, dtype=np.float64)
        log_count = 0

        lin_mean = np.zeros(3, dtype=np.float64)
        lin_M2 = np.zeros(3, dtype=np.float64)
        lin_count = 0
        ...
        for sim_file in self.sim_files:
            ...
            with h5py.File(filepath, 'r') as f:
                timesteps = sorted(list(f['point_clouds'].keys()))
                for ts in timesteps:
                    data = f['point_clouds'][ts][:].astype(np.float64)
                    positions = data[:, :2]
                    fields = data[:, 2:]
                    ...
                    log_fields = np.log10(fields[:, [0, 1, 4]] + self.log_eps)
                    lin_fields = fields[:, [2, 3, 5]]

                    log_count, log_mean, log_M2 = self._update_stats(log_count, log_mean, log_M2, log_fields)
                    lin_count, lin_mean, lin_M2 = self._update_stats(lin_count, lin_mean, lin_M2, lin_fields)

        log_var = log_M2 / max(log_count - 1, 1)
        lin_var = lin_M2 / max(lin_count - 1, 1)

        stats = {
            "pos_min": torch.from_numpy(pos_min).to(torch.float32),
            "pos_max": torch.from_numpy(pos_max).to(torch.float32),
            "dens_log_mean": torch.tensor(log_mean[0], dtype=torch.float32),
            "dens_log_std": torch.tensor(np.sqrt(log_var[0]) + 1e-8, dtype=torch.float32),
            "pres_log_mean": torch.tensor(log_mean[1], dtype=torch.float32),
            "pres_log_std": torch.tensor(np.sqrt(log_var[1]) + 1e-8, dtype=torch.float32),
            "magp_log_mean": torch.tensor(log_mean[2], dtype=torch.float32),
            "magp_log_std": torch.tensor(np.sqrt(log_var[2]) + 1e-8, dtype=torch.float32),
            "velx_mean": torch.tensor(lin_mean[0], dtype=torch.float32),
            "velx_std": torch.tensor(np.sqrt(lin_var[0]) + 1e-8, dtype=torch.float32),
            "vely_mean": torch.tensor(lin_mean[1], dtype=torch.float32),
            "vely_std": torch.tensor(np.sqrt(lin_var[1]) + 1e-8, dtype=torch.float32),
            "magz_mean": torch.tensor(lin_mean[2], dtype=torch.float32),
            "magz_std": torch.tensor(np.sqrt(lin_var[2]) + 1e-8, dtype=torch.float32),
        }
        
        self.logger.info("AMR normalization statistics computed")
        self.logger.info(
            f"  Position range: r=[{pos_min[0]:.4f}, {pos_max[0]:.4f}], z=[{pos_min[1]:.4f}, {pos_max[1]:.4f}]"
        )
        self.logger.info(
            f"  Density log mean/std: {stats['dens_log_mean']:.4f}/{stats['dens_log_std']:.4f}"
        )
        self.logger.info(
            f"  Pressure log mean/std: {stats['pres_log_mean']:.4f}/{stats['pres_log_std']:.4f}"
        )
        self.logger.info(
            f"  Velocity-x mean/std: {stats['velx_mean']:.4f}/{stats['velx_std']:.4f}"
        )
    ...
    @staticmethod
    def _build_stats_id(root: Path, dataset_name: str) -> str:
        root_id = hashlib.md5(str(root.resolve()).encode()).hexdigest()[:12]
        return f"amr_{dataset_name}_{root_id}"

    def _load_stats(self, stats):
        pos_min = stats["pos_min"]
        pos_max = stats["pos_max"]
        pos_range = pos_max - pos_min
        self.pos_offset = pos_min
        self.pos_scale = (1.0 / (pos_range + 1e-8)) * self.pos_scale_factor
        self.pos_scale = self.pos_scale.to(torch.float32)

        self.dens_log_mean = stats["dens_log_mean"]
        self.dens_log_std = stats["dens_log_std"].clamp_min(1e-8)
        self.pres_log_mean = stats["pres_log_mean"]
        self.pres_log_std = stats["pres_log_std"].clamp_min(1e-8)
        self.magp_log_mean = stats["magp_log_mean"]
        self.magp_log_std = stats["magp_log_std"].clamp_min(1e-8)
        self.velx_mean = stats["velx_mean"]
        self.velx_std = stats["velx_std"].clamp_min(1e-8)
        self.vely_mean = stats["vely_mean"]
        self.vely_std = stats["vely_std"].clamp_min(1e-8)
        self.magz_mean = stats["magz_mean"]
        self.magz_std = stats["magz_std"].clamp_min(1e-8)
    ...
    def normalize_fields(self, fields):
        ...
        # Log-normalize
        eps = torch.tensor(self.log_eps, dtype=dens.dtype, device=dens.device)
        dens_norm = (torch.log10(dens + eps) - self.dens_log_mean.to(dens.device)) / self.dens_log_std.to(dens.device)
        pres_norm = (torch.log10(pres + eps) - self.pres_log_mean.to(pres.device)) / self.pres_log_std.to(pres.device)
        magp_norm = (torch.log10(magp + eps) - self.magp_log_mean.to(magp.device)) / self.magp_log_std.to(magp.device)
