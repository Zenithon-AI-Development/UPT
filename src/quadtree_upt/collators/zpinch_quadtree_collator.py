    ...
    def collate(self, batch, dataset_mode, ctx=None):
        ...
        for nodes, snapshot in zip(nodes_per_sample, snapshots):
            ...
            stats = compute_supernode_stats(
                super_mask=super_mask.unsqueeze(0),
                sub_mask=sub_mask.unsqueeze(0),
                sub_depths=sub_depths.unsqueeze(0),
                super_depths=super_depths.unsqueeze(0),
                sub_rel_centers=sub_rel_centers.unsqueeze(0),
                super_half_sizes=super_half_sizes.unsqueeze(0),
                sub_distances=sub_distances.unsqueeze(0),
                max_children=self.max_children or sub_mask.shape[-1],
            ).squeeze(0)
            super_stats_list.append(stats)
        ...
        ctx_out["quadtree_supernodes"] = {
            ...
            "stats": torch.stack(super_stats_list, dim=0),
        }
