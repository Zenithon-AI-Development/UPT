...
try:
    # optional gyro-specific collator
    from datasets.collators.gyro_simformer_collator import GyroSimformerCollator  # type: ignore
except Exception:
    GyroSimformerCollator = None
...
class CfdSimformerTrainer(SgdTrainer):
    def __init__(
        ...
        # loss weights (field vs scalar flux)
    ...
    def input_shape(self):
        ...
        if GyroSimformerCollator is not None:
            assert isinstance(collator.collator, (CfdSimformerCollator, GyroSimformerCollator))
        else:
    ...
    def dataset_mode(self):
        # extended to carry conditioning and dual-horizon targets for gyro setup
        return "x mesh_pos query_pos mesh_edges geometry2d timestep velocity target cond_vec target_h1 target_h5 scalar_target_h1 scalar_target_h5"
    ...
    class Model(nn.Module):
        ...
        def prepare(self, batch, dataset_mode=None, record_timings: bool = False):
            ...
            data = dict(
                ...
                # optional/extended items (present for gyro datasets)
            ...
            # Ensure target_h1 and target_h5 have correct shape (should be [N, C] not [N])
            target_h1 = data.pop("target_h1_raw", None)
            target_h5 = data.pop("target_h5_raw", None)
            # Debug removed temporarily to avoid logger issues
            # Fix target_h1 and target_h5 shapes to match target
            ...
            if mesh_edges is None and not has_quadtree:
                ...
                if record_timings:
                    ...
                    if torch.cuda.is_available():
                        try:
                            ...
                            pass
                # DEBUG: Check edges after creation
                if self.trainer.update_counter.update == 1:
                    num_nodes = mesh_pos.shape[0]
                    num_edges = mesh_edges.shape[1] if mesh_edges.ndim == 2 else 0
                    self.trainer.logger.info(f"[DEBUG] After radius_graph: num_nodes={num_nodes}, num_edges={num_edges}, mesh_edges.shape={mesh_edges.shape if mesh_edges is not None else None}")
                    if mesh_edges is not None and mesh_edges.numel() > 0:
                        unique_nodes = torch.unique(mesh_edges).numel()
                        self.trainer.logger.info(f"[DEBUG] Unique nodes in edges: {unique_nodes}/{num_nodes}")
                        if unique_nodes < num_nodes:
                            self.trainer.logger.warning(f"[DEBUG] WARNING: Not all nodes have edges! {unique_nodes}/{num_nodes} nodes connected")
                    else:
                        self.trainer.logger.error(f"[DEBUG] ERROR: mesh_edges is None or empty!")
                if supernode_idxs is not None:
                    ...
                    if self.trainer.update_counter.update == 1:
                        num_supernode_edges = mesh_edges.shape[1] if mesh_edges.ndim == 2 else 0
                        self.trainer.logger.info(f"[DEBUG] After supernode filtering: num_supernode_edges={num_supernode_edges}")
                ...
                mesh_edges = None
            # DEBUG: Final edge check
            if self.trainer.update_counter.update == 1:
                if mesh_edges is None:
                    self.trainer.logger.error(f"[DEBUG] ERROR: Final mesh_edges is None!")
                elif mesh_edges.numel() == 0:
                    self.trainer.logger.error(f"[DEBUG] ERROR: Final mesh_edges is empty!")
                else:
                    self.trainer.logger.info(f"[DEBUG] Final mesh_edges: shape={mesh_edges.shape}, num_edges={mesh_edges.shape[0] if mesh_edges.ndim == 2 else 0}")
                    if torch.isnan(mesh_edges).any():
                        self.trainer.logger.error(f"[DEBUG] ERROR: NaN values in mesh_edges!")
        ...
        def forward(self, batch, reduction="mean"):
            ...
            def _call_model(horizon_val):
                ...
                if cond_vec is not None:
                    # horizon as [B] float tensor if conditioner supports it
                ...
                return self.model(
                    x,
                    **{k: v for k, v in data.items() if k not in ["target", "target_h1", "target_h5", "scalar_target_h1", "scalar_target_h5", "cond_vec"]},
                    **forward_kwargs,
                    cond_vec=cond_vec,
