...
class CfdSlotSimformerTrainer(CfdSimformerTrainer):
    ...
    def dataset_mode(self):
        return "x mesh_pos query_pos mesh_edges geometry2d constant_fields timestep velocity target"
    ...
    class Model(CfdSimformerTrainer.Model):
        def prepare(self, batch, dataset_mode=None):
            ...
            if "subnode_level" in ctx and ctx["subnode_level"] is not None:
                data["subnode_level"] = ctx["subnode_level"].to(self.model.device, non_blocking=True).long()
            if "slot_positions" in ctx and ctx["slot_positions"] is not None:
                data["slot_positions"] = ctx["slot_positions"].to(self.model.device, non_blocking=True)
            if "subnode_pos" in ctx and ctx["subnode_pos"] is not None:
                data["subnode_pos"] = ctx["subnode_pos"].to(self.model.device, non_blocking=True)
        ...
        def forward(self, batch, reduction="mean"):
            ...
            subnode_level = data.pop("subnode_level", None)
            _slot_positions = data.pop("slot_positions", None)
            subnode_pos = data.pop("subnode_pos", None)
            ...
            model_outputs = self.model(
                ...
                **(dict(subnode_level=subnode_level) if getattr(self.model, "expects_subnode_level", False) else {}),
                **(dict(subnode_pos=subnode_pos) if subnode_pos is not None else {}),
                num_cells=batch_idx_cells.numel(),
            ...
            if prediction.shape != target.shape:
                if prediction.ndim == 2 and target.ndim == 2 and prediction.shape[0] == target.shape[0]:
                    Pc = prediction.shape[1]
                    Tc = target.shape[1]
                    if Tc > Pc and Tc % Pc == 0:
                        target = target[:, -Pc:]
                    elif Pc > Tc and Pc % Tc == 0:
                        prediction = prediction[:, -Tc:]
                if prediction.shape != target.shape:
                    raise RuntimeError(f"prediction/target shape mismatch: pred={tuple(prediction.shape)} "
