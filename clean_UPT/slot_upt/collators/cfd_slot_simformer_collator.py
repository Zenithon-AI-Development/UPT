...
class CfdSlotSimformerCollator(KDSingleCollator):
    ...
    def __init__(self, M=None, N=None, num_supernodes=None, ndim=2, **kwargs):
        """
        Args:
    ...
    def collate(self, batch, dataset_mode, ctx=None):
        ...
        # Determine number of timesteps from features
        # Assume features are [K, T*C] or [K, C]
        num_timesteps = 1
        if collated_batch["x"].ndim == 2:
            # Try to infer timesteps from feature dimension
            # This is dataset-specific - for now, assume single timestep
            # TODO: Make this configurable or infer from dataset
            num_timesteps = 1
        
        # Assign cells to slots using voxel grid
        subnode_feats, subnode_mask, slot2cell, slot_positions = assign_cells_to_slots_voxel_grid(
        ...
        # Store slot representation in context for trainer consumption
        ctx["subnode_feats"] = subnode_feats  # [B, T, M, N, C]
        ctx["subnode_mask"] = subnode_mask  # [B, T, M, N]
        ctx["slot2cell"] = slot2cell  # [B, T, M, N]
        ctx["slot_positions"] = slot_positions  # [M, N, d_x]
