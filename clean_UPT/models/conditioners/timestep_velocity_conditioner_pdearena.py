...
class TimestepVelocityConditionerPdearena(SingleModelBase):
    ...
    def forward(self, timestep, velocity):
        ...
        if velocity is None:
            velocity = timestep.new_zeros(timestep.numel(), 1, dtype=torch.float32)
        else:
            velocity = velocity.view(-1, 1).float()
            if velocity.numel() != timestep.numel():
                if velocity.numel() == 1:
                    velocity = velocity.repeat(timestep.numel(), 1)
                else:
                    raise AssertionError("velocity length mismatch")
        # for rollout timestep is simply initialized as 0 -> repeat to batch dimension
        if timestep.numel() == 1:
            timestep = timestep.repeat(velocity.numel())
        timestep = timestep.clamp_max(self.num_total_timesteps - 1)
        # embed
