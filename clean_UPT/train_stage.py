...
def train_stage(stage_hp: dict, static_config: StaticConfig, cli_args: CliArgs, device: str):
    ...
    def _fallback_input_output_shapes():
        dataset, _ = trainer.data_container.get_dataset("train", mode="x")
        input_shape_local = dataset.getshape_x()
        output_shape_local = dataset.getshape_target()
        logging.info(f"fallback input_shape: {input_shape_local}")
        logging.info(f"fallback output_shape: {output_shape_local}")
        return input_shape_local, output_shape_local

    try:
        input_shape = trainer.input_shape
        output_shape = trainer.output_shape
    except AttributeError:
        input_shape, output_shape = _fallback_input_output_shapes()
